...
button.addEventListener("click", function () {
    ...
    var accuracy = typedWords.length > 0 ? (correctWords / typedWords.length) * 100 : 0;
    var timeTaken = (Date.now() - (startTime || 0)) / 1000 / 60;
    var wpm = timeTaken > 0 ? correctWords / timeTaken : 0;
    result.textContent = "Correct words: ".concat(correctWords, "/").concat(quoteWords.length, ", Accuracy: ").concat(accuracy.toFixed(2), "%, WPM: ").concat(wpm.toFixed(2));
});
